    ...
    for j in range(0, 2):
        for i in range(0, len(relayfinal1[j])):
            if int(modefinal1[j][i]) == 1:
                data["bridges"].append({'serial': {'baudrate': int(bdfinal1[0][i]), 'bytesize': int(bytesizefinal1[0][i]), 'parity': parityfinal1[0][i], 'stopbits': int(stopbitfinal1[0][i]), 'port': '/dev/ttyUSB'+str(indexfinal1[0][i])},
                                        'tcp': {'reuse_addr': True, 'no_delay': True, 'tos': 16, 'listen': 1, 'address': '0:'+portaddressfinal1[0][i]}})
...
#                     cekServer()
                    ...
                    for k in range(0, len(indexfinal2[j])):
                        mycursor = mydb.cursor()
                        sql = "SELECT * FROM device_list WHERE port_device='" + \
